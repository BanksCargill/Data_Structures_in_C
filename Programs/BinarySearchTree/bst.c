...
* Email: cargillb@oregonstate.edu
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    if (cur == 0){
        ...
        //set it's value and it's children's values
        node->val = val;
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    //loop until val is found or current is NULL (not found)
    while (current!=0){
        if (compare(val, current->val)==0){
            return 1;                   //value found
        }
        else if(compare(val, current->val) == -1){
            current = current->left;    //go left if less than
        }
        else{
            current = current->right;   //go right if greater than
        }
    }
...
TYPE _leftMost(struct Node *cur)
{
    /*write this*/
    assert(cur!=NULL);
    //traverse tree until finding leftmost child
    while(cur->left!=NULL){
        cur = cur->left;
    }
    return cur->val;
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    if(compare(cur->val, val)==0){
        //no right child to the removal node, return left child
        if(cur->right == NULL){
            node = cur->left;
            free(cur);
            return node;
    ...
    //value not found: call function on left and right children of the root
