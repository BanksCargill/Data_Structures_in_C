...
void listQueueAddBack (struct Queue* queue, TYPE value)
{
    ...
    //set newLink value and it's next to NULL
    newLink->value = value;
    newLink->next = NULL;
    //tail should now point to the newLink
...
TYPE listQueueRemoveFront(struct Queue* queue)
{
    ...
    assert(queue->head->next!=NULL);
    //copy link
    struct Link *first = queue->head->next;
    //remove link
    //edge case (if only one link)
    if(queue->head->next == queue->tail){
        queue->tail = queue->head;
        queue->head->next = NULL;
    }
    //otherwise, head's next points to its next, next. (confusing way to word it)
    else{
        queue->head->next = queue->head->next->next;
    }
    //copy value to enable removal and freeing of copy link
...
void listSwapStackQueues(struct Stack* stack)
{
    ...
    //create temp and swap q's
    struct Queue *q3 = stack->q1;
    stack->q1 = stack->q2;
    stack->q2 = q3;
}
...
void listStackPush(struct Stack* stack, TYPE value)
{
	/* FIXME: You will write this function */
	assert(stack!=NULL);
	//new link created and added to end of q2
	listQueueAddBack(stack->q2, value);
	//front of q1 removed and added to end of q2 (until q1 is empty)
...
TYPE listStackPop(struct Stack* stack)
{
	...
	//remove link and set variable to value held
	double value = listQueueRemoveFront(stack->q1);
