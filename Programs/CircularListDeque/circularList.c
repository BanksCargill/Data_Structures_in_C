...
static void init(struct CircularList* deque)
{
    ...
    //set next and prev to point to sentinel
...
static void removeLink(struct CircularList* deque, struct Link* link)
{
	...
	assert(deque!=0);
	assert(link!=0);
	//reassign next and previous for link before and after
...
void circularListPrint(struct CircularList* deque)
{
    ...
    struct Link *newLink = deque->sentinel->next;
    while(newLink != deque->sentinel){
        printf("%g ", newLink->value);
        newLink = newLink->next;
    }
    //print statement for aesthetics of testing
    printf("\n");
...
{
	/* FIXME: You will write this function */
	//no allocation of new memory allowed
	assert(deque!=0);
	assert(!circularListIsEmpty(deque));
